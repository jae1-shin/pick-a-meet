...
- 사용자 등록·수정과 권한 관리
- 사용자 이름 옆 전환 아이콘을 통한 Admin 사용자 전환과 원래 Admin 세션 복귀
- 모임 생성·수정, Host 배정과 상태 관리
- 신청 시작 시각과 Host 정보 공개 여부를 각각 저장하고 대기 화면 미리보기
- 사용자·모임 관리 테이블 정렬
- 추가 Admin console 비밀번호 확인

사용자 전환 세션은 일반 로그인 세션과 명시적으로 구분합니다. 전환 중에는 대상 사용자의 최신 권한으로 동작하고 `로그아웃` 대신 `Admin으로 돌아가기`가 표시됩니다. 복귀 시 원래 계정이 여전히 활성 Admin인지 서버에서 다시 확인합니다.
...
`seed_demo`는 로컬의 사용자·로그인 이력·모임·신청 데이터를 초기화한 뒤 아래 개발 데이터를 다시 만듭니다. 운영 DB에서는 절대 실행하지 않습니다. Admin이 설정한 전역 신청 시작 시각은 초기화하지 않습니다.
...
| 역할 | ID | 사번 | 비고 |
|---|---|---|---|
| Admin + Host | `leader01` | `1` | 겸임 권한 확인 |
| Host 전용 | `leader02`~`leader18` | `2`~`18` | 리더마다 모임 하나 담당 |
| Admin 전용 | `member19`, `member20` | `19`, `20` | 일반 구성원 중 Admin 2명 |
| 일반 사용자 | `member21`~`member120` | ID와 같은 숫자 | 여러 파트·모듈 |

- 모든 더미 계정은 ID 끝 숫자와 사번이 같습니다.
- 파트별 활성 인원은 `32 / 30 / 28 / 15 / 15명`입니다. 앞의 세 파트는 모임 수 18개보다 많고, 뒤의 두 파트는 적습니다.
- 모임은 18개이며 리더 18명이 각각 하나씩 담당합니다. 샘플 신청에는 같은 파트 2명 허용 사례가 포함됩니다.
