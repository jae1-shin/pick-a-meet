...
예외:
해당 파트의 활성 Y 인원 > OPEN + CLOSED 모임 수
→ 같은 파트 2명까지 허용

신청 가능 여부는 파트원 수 계산에 사용하지 않음

단:
한 모임에서 2명이 신청한 파트는 하나만 허용
같은 파트 3명째는 항상 차단
```

이 Rule의 최종 검증은 대상 Meeting Row Lock을 획득한 뒤 최신 신청자 기준으로 수행한다.
...
4. 일반 사용자에게 신청자 사번은 노출하지 않는다. Host 이름·ID·파트·모듈은 Admin의 전역 공개 설정이 켜진 경우에만 노출한다.
